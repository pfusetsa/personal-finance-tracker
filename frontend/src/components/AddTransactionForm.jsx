...
function AddTransactionForm({ accounts, categories, onFormSubmit, onCancel, lang }) {
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], description: '', amount: '', currency: 'EUR', is_recurrent: false, account_id: accounts[0]?.id || '', category_id: categories[0]?.id || '', });
