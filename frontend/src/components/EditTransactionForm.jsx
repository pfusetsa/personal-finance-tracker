...
function EditTransactionForm({ transaction, accounts, categories, onFormSubmit, onCancel, lang }) {
  ...
  const [formData, setFormData] = useState({ date: transaction.date, description: transaction.description, amount: transaction.amount, currency: transaction.currency || 'EUR', is_recurrent: transaction.is_recurrent, account_id: transaction.account_id, category_id: transaction.category_id, });
  const handleChange = (e) => { const { name, value, type, checked } = e.target; setFormData(prev => ({ ...prev, [name]: type === 'checkbox' ? checked : value })); };
  const handleSubmit = (e) => { e.preventDefault(); onFormSubmit(transaction.id, formData); };
